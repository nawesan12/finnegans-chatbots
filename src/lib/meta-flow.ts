...
const performRequest = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new MetaFlowError(message, { status: response.status, details: json });
    }
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new MetaFlowError("Meta Flow API request timed out", { status: 504 });
    }
...
export const updateMetaFlow = async (
  ...
  if (!flowId?.trim()) {
    throw new MetaFlowError("Missing remote flow id for update", { status: 400 });
  }
  const credentials = await ensureCredentials(userId);
...
export const deleteMetaFlow = async (
  ...
  if (!flowId?.trim()) {
    throw new MetaFlowError("Missing remote flow id for delete", { status: 400 });
  }
  const credentials = await ensureCredentials(userId);
